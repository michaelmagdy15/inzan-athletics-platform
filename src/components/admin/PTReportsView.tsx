...
export default function PTReportsView() {
  ...
  const sessionsPerCoach = useMemo(() => {
    return coaches.map((c) => ({
      name: c.name.split(" ")[0],
      completed: ptSessions.filter(
        (s) => s.coach_id === c.id && s.status === "completed",
      ).length,
      scheduled: ptSessions.filter(
        (s) => s.coach_id === c.id && s.status === "scheduled",
      ).length,
      canceled: ptSessions.filter(
        (s) => s.coach_id === c.id && s.status === "canceled",
      ).length,
      noShow: ptSessions.filter(
        (s) => s.coach_id === c.id && s.status === "no_show",
      ).length,
    }));
  }, [ptSessions, coaches]);
  ...
  const clientBalances = useMemo(() => {
    const balanceMap: Record<
      string,
      { name: string; remaining: number; total: number }
    > = {};
    ptPackages
      .filter((p) => p.status === "active")
      .forEach((p) => {
        if (!balanceMap[p.member_id]) {
          balanceMap[p.member_id] = {
  ...
  const completedSessions = ptSessions.filter(
  ...
  const handleDownloadReport = () => {
    ...
    const rows = clientBalances.map((c) => [
      ...
      c.total,
      c.total > 0
        ? Math.round(((c.total - c.remaining) / c.total) * 100) + "%"
        : "0%",
    ]);
    const csvContent = [
      headers.join(","),
    ...
    <div className="flex flex-col gap-6 lg:gap-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors border border-white/5 text-[9px] uppercase tracking-widest font-black text-white/60 hover:text-white w-full sm:w-auto"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            value: ptPackages.filter((p) => p.status === "active").length,
          ...
          >
            ...
            <p className={`text-2xl lg:text-3xl font-heading ${kpi.color}`}>
              {kpi.value}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card rounded-[2rem] border border-white/5 p-6">
          <h3 className="text-[10px] font-black text-white/30 tracking-[0.4em] uppercase mb-6">
            Sessions Per Coach
          </h3>
      ...
      <div className="glass-card rounded-[2rem] border border-white/5 p-6">
        <h3 className="text-[10px] font-black text-white/30 tracking-[0.4em] uppercase mb-6">
          Client Session Balances (Lowest First)
        </h3>
            ...
            <thead>
              <tr className="border-b border-white/5">
                {["Client", "Remaining", "Total", "Usage"].map((h) => (
                  <th
                    key={h}
                    className="text-[9px] text-white/30 uppercase tracking-widest font-bold text-left px-5 py-3"
                  >
                    {h}
                  </th>
                ))}
              ...
              {clientBalances.map((c, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className={`px-5 py-3 text-sm font-bold ${c.remaining <= 2 ? "text-red-400" : "text-emerald-400"}`}
                    >
                      {c.remaining}
                    </td>
