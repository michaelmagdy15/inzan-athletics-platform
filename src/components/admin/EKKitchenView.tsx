...
import { ShoppingCart, AlertCircle, TrendingUp, Package, ChevronRight, Zap, Target, ArrowUpRight, Plus, RefreshCcw } from 'lucide-react';
                ...
                {goals.map((goal, idx) => (
                    ...
                    >
                        ...
                        <div className="flex justify-between mt-3 text-[8px] font-bold text-white/20 uppercase tracking-widest">
                            <span>{Math.round((goal.current / goal.target) * 100)}% Fulfilled</span>
                            ...
                            <table className="w-full text-left border-collapse font-bold min-w-[500px]">
                                <thead className="bg-[#0c0c0c]/50">
                                    <tr>
                                        ...
                                        <th className="px-10 py-6 text-[9px] text-white/20 uppercase tracking-[0.4em] font-black border-b border-white/5">Category</th>
                                        <th className="px-10 py-6 text-[9px] text-white/20 uppercase tracking-[0.4em] font-black border-b border-white/5 text-right">Available</th>
                                    </tr>
                                </thead>
                                        ...
                                        >
                                            <td className="px-10 py-8">
                                                <div className="flex flex-col gap-1">
                                                    <span className="font-heading text-lg tracking-tight text-white group-hover:text-gold transition-colors">{item.name}</span>
                                                    <span className="text-[10px] text-white/10 uppercase font-black tracking-widest italic">{item.price} EGP / Unit</span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-10 py-8 text-right">
                                                <div className="flex flex-col items-end gap-2">
                                                    <span className={`text-xl font-heading ${item.quantity <= item.reorder_threshold ? 'text-red-500' : 'text-white'}`}>{item.quantity} <span className="text-[10px] text-white/20 uppercase italic ml-1">{item.unit}</span></span>
                                                    <div className={`w-32 h-1 rounded-full bg-white/5 overflow-hidden`}>
                                                        <div className={`h-full ${item.quantity <= item.reorder_threshold ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]' : 'bg-gold/40'}`} style={{ width: `${Math.min((item.quantity / (item.reorder_threshold * 2)) * 100, 100)}%` }} />
                                                    </div>
                                                </div>
                                            </td>
                                        </motion.tr>
                        ...
                        <div className="flex flex-col gap-6">
                            {orders.slice(0, 4).map((o, idx) => (
                                ...
                                >
                                    ...
                                    <h4 className="text-lg font-heading text-white mb-4 group-hover:text-gold transition-colors leading-tight">1x {o.items?.[0]?.name || 'Pre-Fuel Formula'}</h4>
                                    <div className="flex items-center justify-between gap-4">
                                        <div className="w-8 h-8 rounded-lg bg-white/5 border border-white/5 flex items-center justify-center text-[10px] font-heading text-white/20 font-bold italic">MK</div>
                                        <button
