...
export default function SessionPoliciesView() {
  ...
  const [activeTab, setActiveTab] = useState<"policies" | "memberships">(
    "policies",
  );
  ...
  const iconForField = (field: string) => {
    if (field.includes("cancel"))
      return <XCircle size={14} className="text-red-400" />;
    if (field.includes("reschedule"))
      return <RefreshCw size={14} className="text-purple-400" />;
    if (field.includes("no_show"))
        ...
        <div className="flex flex-col gap-6">
          <div className="glass-card rounded-[2rem] border border-white/5 p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/40">
                Set cancellation windows, no-show rules, reschedule limits, and
                package validity per session type.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {sessionPolicies.map((policy) => (
              <div
                key={policy.id}
                className="glass-card rounded-[2rem] border border-white/5 p-6 flex flex-col gap-5"
              >
                <div className="flex items-center justify-between">
                  <span className="text-[10px] bg-[#FFB800]/10 text-[#FFB800] px-4 py-1.5 rounded-full font-bold tracking-widest uppercase border border-[#FFB800]/20">
                    {SESSION_TYPE_LABELS[policy.session_type as SessionType] ||
                      policy.session_type}
                  </span>
                  {editingId !== policy.id ? (
                    <button
                      onClick={() => startEdit(policy.id)}
                      className="text-[9px] text-white/40 font-bold tracking-widest uppercase hover:text-[#FFB800] transition-colors"
                    >
                      Edit
                    </button>
                  ) : (
                    <button
                      onClick={handleSave}
                      disabled={saving}
                      className="premium-button px-4 py-2 rounded-xl flex items-center gap-2"
                    >
                      <Save size={12} />
                    ...
                    {editingId === policy.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          min={0}
                          value={
                            editValues[field.key] ?? (policy as any)[field.key]
                          }
                          onChange={(e) =>
                            setEditValues({
                              ...editValues,
                  ...
                  {editingId === policy.id ? (
                    <button
                      onClick={() =>
                        setEditValues({
                          ...editValues,
                          no_show_deducts: !editValues.no_show_deducts,
                        })
                      }
                      className={`px-4 py-2 rounded-xl text-[9px] font-bold tracking-widest uppercase border transition-colors ${
                        editValues.no_show_deducts
                    ...
                    <span
                      className={`text-sm font-bold ${policy.no_show_deducts ? "text-emerald-400" : "text-red-400"}`}
                    >
            ...
            {sessionPolicies.length === 0 && (
              <div className="col-span-2 text-center text-white/20 text-sm py-12">
                No policies configured. Run the SQL migration to seed default
                policies.
              </div>
            )}
          </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="flex justify-between items-center bg-black/40 p-6 rounded-[2rem] border border-white/5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-white/40">
                  Manage recurring subscription plans and facility access
                  levels.
                </p>
              </div>
            </div>
            ...
            <div className="glass-card rounded-[2rem] border border-white/5 p-6 mb-6">
              ...
              >
                <input
                  ...
                  type="text"
                  required
                  placeholder="TIER NAME (E.G. ELITE ACCESS)"
                  className="bg-white/5 border border-white/10 rounded-xl p-4 text-sm text-white focus:border-gold/30"
                />
                <div className="flex gap-4">
                  <input
                    ...
                    className="flex-1 bg-white/5 border border-white/10 rounded-xl p-4 text-sm text-white focus:border-gold/30"
                  />
                  <select
                    name="billing_cycle"
                    className="w-1/3 bg-white/5 border border-white/10 rounded-xl p-4 text-sm text-white focus:border-gold/30"
                  >
                ...
                <input
                  ...
                  className="md:col-span-2 bg-white/5 border border-white/10 rounded-xl p-4 text-sm text-white focus:border-gold/30"
                />
                <div className="md:col-span-2 flex gap-4 mt-2">
                  <button
                    type="submit"
                    className="flex-1 bg-[#FFB800] text-black font-bold uppercase tracking-widest text-[10px] rounded-xl py-3 shadow-lg shadow-gold/20"
                  >
                    Save Tier
                  </button>
                  <button
                    ...
                    className="px-6 text-white/30 uppercase text-[10px] font-bold tracking-widest hover:text-white"
                  >
                    Cancel
                  </button>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {membershipTiers.map((tier) => (
              <div
                key={tier.id}
                className="glass-card flex flex-col justify-between rounded-[2rem] p-8 border border-white/5 relative overflow-hidden group"
              >
                ...
                <div>
                  <div className="flex justify-between items-start mb-6">
                    <h3 className="text-lg font-heading tracking-widest text-white uppercase">
                      {tier.name}
                    </h3>
                    <button
                  ...
                  <div className="mb-6">
                    <span className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-br from-[#FFB800] to-[#FF8A00]">
                      {tier.price}
                    </span>
                    <span className="text-white/40 text-xs ml-1 uppercase">
                      EGP / {tier.billing_cycle === "monthly" ? "MO" : "YR"}
                    </span>
                  </div>
