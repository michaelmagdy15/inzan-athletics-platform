...
export default function PTSessionsView() {
    ...
    return ptSessions.filter((s) => {
      ...
      if (searchTerm) {
        ...
        return (
          ...
          s.coach_name?.toLowerCase().includes(term)
        );
      }
      return true;
    });
  ...
  const todayNoShows = todaySessions.filter(
    (s) => s.status === "no_show",
  ).length;
  const totalCanceled = ptSessions.filter(
    (s) => s.status === "canceled",
  ).length;
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        <div>
          ...
          <p className="text-[10px] tracking-[0.4em] text-white/30 uppercase font-medium">
            Front Desk Session Management
          </p>
        </div>
        <button
          onClick={() => setShowAddModal(true)}
          className="flex items-center justify-center gap-2 premium-button px-6 py-3 rounded-xl text-[9px] font-black tracking-widest uppercase text-black w-full lg:w-auto"
        >
            ...
            <thead>
              <tr className="border-b border-white/5">
                {[
                  ...
                  <th
                    key={h}
                    className="text-[9px] text-white/30 uppercase tracking-widest font-bold text-left px-5 py-4"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr>
                  <td
                    colSpan={7}
                    className="text-center text-white/20 text-sm py-12"
                  >
                    No sessions found.
                  </td>
                </tr>
              ...
              {filtered.map((session) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-[10px] text-white/40 uppercase tracking-widest">
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-[8px] font-bold tracking-widest uppercase border ${cfg.color}`}
                      >
                        {cfg.label}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      {session.status === "scheduled" && (
                        <div className="flex gap-1">
                          <button
                            onClick={() =>
                              handleAction(session.id, "completed")
                            }
            ...
            >
              ...
              <div className="flex flex-col gap-4 font-bold">
                <select
                  name="member_id"
                  required
                  className="w-full bg-black/40 border border-white/5 rounded-2xl p-5 text-sm outline-none text-white appearance-none cursor-pointer focus:border-gold/30"
                >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input
                    ...
                    type="date"
                    required
                    className="w-full bg-black/40 border border-white/5 rounded-2xl p-5 text-sm outline-none text-white focus:border-gold/30"
                  />
                  <input
                    name="time"
                    type="time"
                ...
                <input
                  ...
                  className="w-full bg-black/40 border border-white/5 rounded-2xl p-5 text-sm outline-none text-white focus:border-gold/30 placeholder-white/20"
                />
              </div>
              ...
              <div className="flex gap-4 lg:gap-6 pt-2 font-bold">
                ...
                <button
                  type="submit"
                  className="premium-button flex-1 h-14 rounded-2xl text-black tracking-[0.3em] uppercase text-[10px] shadow-2xl shadow-gold/20"
                >
                  Confirm Booking
                </button>
              </div>
