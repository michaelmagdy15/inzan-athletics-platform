    ...
    <div className="flex flex-col gap-6 lg:gap-10">
        ...
        {lowestAttendanceClass && Number(insightCapacity) < 50 && (
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-start lg:items-center gap-6 lg:gap-8 relative z-10 w-full lg:w-auto">
              ...
              <div className="max-w-xl">
                ...
                <p className="text-sm text-white/70 leading-relaxed font-light">
                  {insightDay} {insightTime}{" "}
                  <span className="text-white font-medium">
                    "{insightTitle}"
                  </span>{" "}
                  classes are showing low attendance ({insightCapacity}%
                  capacity).{" "}
                  <span className="text-white font-medium">
            ...
            <button
              ...
              className="w-full lg:w-auto premium-button px-8 lg:px-10 py-4 lg:py-5 rounded-2xl transition-all hover:scale-[1.05] relative z-10 shadow-2xl shadow-gold/20"
            >
      ...
      <div className="glass-card rounded-[2rem] lg:rounded-[3rem] border border-white/5 p-5 lg:p-10 shadow-2xl flex flex-col gap-6 lg:gap-8 relative overflow-hidden">
        ...
        <div
          className={
            viewMode === "List"
              ? "flex flex-col gap-4 font-bold"
              : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 font-bold"
          }
        >
          {classes.map((c, idx) => (
            <motion.div
              ...
              >
                <div className="flex flex-col gap-1 text-left sm:text-right">
                  <div className="text-sm font-mono tracking-[0.2em] text-white/80">
                    <span
                      className={
                        c.spots_left === 0 ? "text-red-500" : "text-gold"
                      }
                    >
          ...
          <>
            ...
            >
              <div className="p-6 lg:p-8 border-b border-white/5 flex justify-between items-center bg-black/20">
                <div>
                  ...
                  <p className="text-[10px] text-gold tracking-widest font-bold uppercase">
                    {selectedClass.time} • {selectedClass.trainer}
                  </p>
                </div>
                <button
                  onClick={() => setSelectedClass(null)}
              ...
              <div className="p-6 lg:p-8 flex flex-col gap-6 overflow-y-auto flex-1 scrollbar-hide">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/5 rounded-2xl p-4 flex flex-col items-center justify-center border border-white/5">
                    <span className="text-2xl font-heading text-white mb-1">
                      {selectedClass.total_spots - selectedClass.spots_left}
                    </span>
                ...
                <div>
                  <h4 className="text-[10px] font-black text-white/40 tracking-[0.3em] uppercase mb-4">
                    Roster
                  </h4>
                  {enrolledMembers.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-3">
                      {enrolledMembers.map((member) => (
                        <div
                          key={member.id}
                          className="flex items-center gap-4 p-4 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-gold/20 transition-colors"
                        >
                          <img
                            src={member.avatar}
                            alt={member.name}
                          ...
                          <div className="flex flex-col flex-1 min-w-0">
                            ...
                            <span className="text-[9px] text-white/30 uppercase tracking-widest">
                              {member.role} • {member.membershipStatus}
                            </span>
                          </div>
                        </div>
                      ))}
