        ...
        <div className="flex flex-col gap-10">
            ...
            >
                ...
                <div className="flex items-start gap-8 relative z-10">
                    <div className="w-16 h-16 rounded-2xl bg-gold/10 flex items-center justify-center border border-gold/20 shadow-[0_0_30px_rgba(202,138,4,0.1)] group-hover:scale-110 transition-transform duration-700">
            ...
            <div className="flex gap-4 p-2 bg-black/40 rounded-3xl border border-white/5 font-bold">
                {days.map(day => (
                    <button
                        ...
                        className={`flex-1 py-5 rounded-2xl text-[10px] font-black tracking-[0.4em] transition-all duration-500 relative overflow-hidden group ${activeDay === day
                            ? 'text-white'
                ...
                <div className="flex flex-col gap-4 font-bold">
                    {classes.map((c, idx) => (
                        <motion.div
                            ...
                            key={c.id}
                            className="group bg-white/[0.02] hover:bg-white/[0.05] border border-white/5 hover:border-gold/20 rounded-3xl p-8 flex justify-between items-center transition-all duration-500 cursor-pointer shadow-inner relative overflow-hidden"
                        >
                            ...
                            <div className="flex items-center gap-8 relative z-10">
                                <div className="flex flex-col items-center">
                                    <span className="text-2xl font-heading text-white group-hover:text-gold transition-colors">{c.time.split(' ')[0]}</span>
                                    <span className="text-[9px] text-white/20 font-black tracking-widest uppercase">{c.time.split(' ')[1]}</span>
                                </div>
                                <div className="w-px h-12 bg-white/5" />
                                <div className="flex flex-col gap-1.5">
                                    <h4 className="text-xl font-heading text-white tracking-tight uppercase group-hover:text-gold transition-colors">{c.title}</h4>
