...
export default function InventoryView() {
  ...
  const operationalMachines = equipment.filter(
  ...
  const auditScore =
    facilityZones.length > 0
      ? Math.round((safeZones / facilityZones.length) * 100)
      : 100;

  // Lifespan calculations
  const nowYear = new Date().getFullYear();
  const sumAges = equipment.reduce(
    (acc, e) => acc + (nowYear - new Date(e.purchase_date).getFullYear()),
    0,
  );
  const avgAge = totalMachines > 0 ? (sumAges / totalMachines).toFixed(1) : "0";

  const acquisitionsNeeded = equipment.filter(
    (e) =>
      nowYear - new Date(e.purchase_date).getFullYear() >=
      e.expected_lifespan_years,
  ).length;
  const maxLifespan =
    totalMachines > 0
      ? Math.max(...equipment.map((e) => e.expected_lifespan_years))
      : 10;
  const ageProgressWidth = Math.min(
    100,
    (Number(avgAge) / (maxLifespan || 10)) * 100,
  );
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        <div>
          ...
          <p className="text-[10px] tracking-[0.3em] text-white/30 uppercase font-medium">
            Gym Equipment & Facility Logs
          </p>
        </div>
        <div className="flex items-center gap-3 lg:gap-4 font-bold w-full lg:w-auto">
          <button
            onClick={() => broadcastAlert("Facility status refreshed.", "info")}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
        ...
        <div className="xl:col-span-2 flex flex-col gap-8">
          <div className="glass-card rounded-[3rem] border border-white/5 overflow-hidden shadow-2xl relative">
            <div className="px-10 py-8 border-b border-white/5 flex justify-between items-center bg-black/20">
              ...
              <button
                onClick={() => setIsLogModalOpen(true)}
                className="w-10 h-10 rounded-full bg-white/5 border border-white/5 flex items-center justify-center text-white/20 hover:text-gold transition-all"
              >
                <Plus size={18} />
              </button>
            </div>
        ...
        <div className="flex flex-col gap-10">
            ...
            <div className="space-y-8">
              <div className="flex flex-col gap-4">
                ...
                <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gold/40 shadow-[0_0_10px_rgba(202,138,4,0.2)] transition-all duration-1000"
                    style={{ width: `${ageProgressWidth}%` }}
                  />
                </div>
              </div>
              ...
              <p className="text-[11px] text-white/40 leading-relaxed uppercase tracking-widest font-bold">
                ...
                <span className="text-white">
                  {acquisitionsNeeded} equipment replacement(s)
                </span>{" "}
                suggested for{" "}
                <span className="text-gold italic">this year</span>.
              </p>
              ...
              <button className="w-full py-5 bg-white/5 hover:bg-gold hover:text-black border border-white/5 hover:border-gold rounded-2xl flex justify-between items-center px-8 transition-all duration-500 group/btn font-bold">
                <span className="text-[9px] font-black tracking-[0.3em] uppercase opacity-40 group-hover/btn:opacity-100 transition-opacity">
                  Order Acquisition
                </span>
                <ChevronRight
                  size={14}
                  className="text-white/10 group-hover/btn:text-black transition-all"
                />
              </button>
            </div>
          </div>
          ...
          <div className="glass-card rounded-[3rem] border border-white/5 p-10 shadow-2xl relative overflow-hidden font-bold">
            <h3 className="text-[10px] font-black text-white/30 tracking-[0.4em] uppercase mb-10">
              Facility Index
            </h3>
              ...
              {facilityZones.length > 0 ? (
                facilityZones.map((zone) => (
                  <ZoneStatus
                    ...
                    label={zone.name}
                    status={zone.status}
                    health={zone.health_percentage}
                    warning={zone.requires_warning}
                  />
                ))
              ) : (
            ...
            >
              ...
              <div className="flex flex-col gap-6">
                <input
                  name="asset"
                  required
                  placeholder="EQUIPMENT NAME (e.g. Treadmill X1)"
                ...
                <div className="relative group">
                  ...
                  >
                    ...
                    <option value="in_progress" className="bg-black">
                      In Progress
                    </option>
...
function Scorecard({ title, value, trend, icon, highlight }: any) {
  return (
    <div
      className={`p-6 lg:p-10 rounded-[2rem] lg:rounded-[3rem] border relative overflow-hidden transition-all duration-700 group cursor-pointer font-bold ${highlight ? "bg-gold/10 border-gold/20 shadow-[0_0_60px_rgba(202,138,4,0.15)]" : "bg-[#121212]/30 border-white/5 hover:border-gold/20"}`}
    >
      <div className="flex justify-between items-start mb-6 lg:mb-8">
        <div className="text-white/20 group-hover:text-gold transition-colors duration-500">
          {icon}
        </div>
        <span
          className={`text-[8px] font-black tracking-widest uppercase px-2 py-1 rounded-lg ${highlight ? "bg-gold text-black" : "bg-white/5 text-white/40 group-hover:text-white transition-colors"}`}
        >
          {trend}
        </span>
      </div>
      <h3 className="text-[9px] lg:text-[10px] font-black tracking-[0.3em] text-white/20 uppercase mb-3 lg:mb-4 group-hover:text-gold transition-colors font-bold">
        {title}
      </h3>
      <span
...
function LogItem({ title, desc, time, initial, highlight }: any) {
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white/[0.02] border border-white/5 rounded-[1.5rem] lg:rounded-[2rem] p-5 lg:p-8 hover:bg-white/[0.04] hover:border-gold/20 transition-all duration-500 group cursor-pointer relative overflow-hidden font-bold gap-6">
      <div
        className={`absolute top-0 left-0 w-1 h-full transition-all duration-500 ${highlight ? "bg-gold/40" : "bg-gold/0 group-hover:bg-gold/20"}`}
      />
      <div className="flex items-center gap-4 lg:gap-8 relative z-10">
        <div className="w-12 h-12 lg:w-16 lg:h-16 rounded-xl lg:rounded-2xl bg-white/[0.05] border border-white/5 flex items-center justify-center text-lg font-heading text-white/20 group-hover:text-gold transition-colors shadow-inner">
          {initial}
        </div>
        <div>
          <h4 className="text-lg lg:text-xl font-heading text-white group-hover:text-gold transition-colors">
            {title}
          </h4>
          <p className="text-[10px] lg:text-[11px] text-white/40 font-light max-w-md tracking-wide uppercase">
            {desc}
          </p>
        </div>
      </div>
      <div className="w-full sm:w-auto text-right flex items-center justify-between sm:justify-end gap-6 lg:gap-10 relative z-10 transition-all">
        <div>
          <p
            className={`text-[9px] font-black uppercase tracking-[0.2em] ${statusColor} transition-colors`}
          >
            {time}
          </p>
        </div>
  ...
  return (
    <div className="flex flex-col gap-2 font-bold">
      <div className="flex justify-between text-[9px] uppercase tracking-widest font-black">
        ...
        <span className={warning ? "text-red-500" : "text-emerald-400"}>
          {status}
        </span>
      </div>
      <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
        <div
          className={`h-full ${warning ? "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]" : "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]"}`}
          style={{ width: `${health}%` }}
        />
      </div>
    </div>
