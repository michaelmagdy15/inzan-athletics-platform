    ...
    return (
        <div className="flex flex-col gap-6 lg:gap-10">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
                <div>
                    <h1 className="text-2xl lg:text-3xl font-heading tracking-tight text-white mb-1 uppercase">Financial Management</h1>
...
function Scorecard({ title, value, currency, trend, subtitle, highlight, alert, icon }: any) {
    return (
        <div className={`p-6 lg:p-10 rounded-[2rem] lg:rounded-[2.5rem] border relative overflow-hidden transition-all duration-700 group cursor-pointer font-bold ${highlight ? 'bg-gold/10 border-gold/20 shadow-[0_0_50px_rgba(202,138,4,0.1)]' :
            ...
            <div className="flex justify-between items-start mb-6 lg:mb-8">
                ...
                {trend && (
                    <span className="text-[9px] lg:text-[10px] font-black text-emerald-400 bg-emerald-400/5 px-2.5 py-1 rounded-lg italic">{trend}</span>
                )}
                {alert && (
                    <span className="text-[9px] lg:text-[10px] font-black text-red-500 bg-red-500/5 px-2.5 py-1 rounded-lg italic animate-pulse">ATTENTION</span>
                )}
            </div>

            <h3 className={`text-[9px] lg:text-[10px] font-black tracking-[0.3em] uppercase mb-4 lg:mb-5 group-hover:text-gold transition-colors ${alert ? 'text-red-500/40' : 'text-white/20'}`}>{title}</h3>
            <div className="flex items-baseline gap-2">
                <span className={`text-4xl lg:text-5xl font-heading tracking-tight ${highlight ? 'text-gold' : alert ? 'text-white underline decoration-red-500/20' : 'text-white'}`}>{value}</span>
            ...
            {subtitle && (
                <p className={`text-[8px] uppercase tracking-[0.3em] mt-3 lg:mt-4 font-bold ${alert ? 'text-red-500/30' : 'text-white/20'}`}>{subtitle}</p>
            )}
        </div>
    );
}
    ...
    return (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white/[0.02] border border-white/5 rounded-[1.5rem] lg:rounded-[2rem] p-6 lg:p-8 hover:bg-white/[0.04] hover:border-gold/20 transition-all duration-500 group cursor-pointer relative overflow-hidden font-bold gap-6">
            ...
            <div className="flex items-center gap-4 lg:gap-8 relative z-10">
                <div className="w-12 h-12 lg:w-16 lg:h-16 rounded-xl lg:rounded-2xl bg-white/[0.05] border border-white/5 flex items-center justify-center text-lg lg:text-xl font-heading text-white/20 group-hover:text-gold transition-colors shadow-inner shrink-0">{initial}</div>
                <div>
