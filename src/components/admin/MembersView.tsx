      ...
      <div className="glass-card rounded-[3rem] border border-white/5 overflow-hidden shadow-2xl relative">
        <div className="px-10 py-8 border-b border-white/5 flex justify-between items-center bg-black/20 font-bold">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-[10px] font-black text-white/30 tracking-[0.3em] uppercase">
              List Overview
            </h3>
          </div>
          <div className="flex gap-4">
            <button
              ...
              className="flex items-center gap-2 px-6 py-2.5 bg-white/5 rounded-xl text-[9px] font-bold text-white/40 tracking-widest uppercase border border-white/5 hover:text-white hover:border-gold/30 transition-all"
            >
              <Filter size={12} /> Filter
            </button>
            <button className="px-6 py-2.5 bg-gold/10 rounded-xl text-[9px] font-black text-gold tracking-widest uppercase border border-gold/20">
              {statusFilter === "all"
                ? "All Status"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="bg-[#0c0c0c]/50 font-bold">
              <tr>
                <th className="px-10 py-6 text-[9px] text-white/20 uppercase tracking-[0.3em] font-black border-b border-white/5">
                  Member Name
                </th>
                <th className="px-10 py-6 text-[9px] text-white/20 uppercase tracking-[0.3em] font-black border-b border-white/5">
                  Role
                ...
                <th className="px-10 py-6 text-[9px] text-white/20 uppercase tracking-[0.3em] font-black border-b border-white/5 text-right">
                  Risk
                </th>
              </tr>
            </thead>
              ...
              {displayedMembers.map((member, idx) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-5">
                      <div className="relative group/avatar">
                        ...
                        <img
                          src={member.avatar}
                          alt=""
                          className="w-12 h-12 rounded-2xl object-cover ring-2 ring-white/5 shadow-2xl relative z-10"
                        />
                      </div>
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className="text-[9px] text-white/20 uppercase tracking-widest font-bold group-hover:text-white/40 transition-colors italic">
                  ...
                  <td className="px-10 py-8">
                    <span className="text-[10px] text-white/40 tracking-[0.1em] font-light uppercase group-hover:text-white/60 transition-colors italic">
                      {member.role}
                    </span>
                  </td>
                  <td className="px-10 py-8">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-1.5 h-1.5 rounded-full animate-pulse shadow-[0_0_8px_rgba(255,255,255,0.2)] ${member.membershipStatus === "active" ? "bg-emerald-400" : member.membershipStatus === "suspended" ? "bg-amber-500" : "bg-red-500"}`}
                      />
                      <span
                        className={`text-[10px] font-black tracking-[0.2em] uppercase ${member.membershipStatus === "active" ? "text-emerald-400" : member.membershipStatus === "suspended" ? "text-amber-500" : "text-red-500"}`}
                      >
                  ...
                  <td className="px-10 py-8 text-right">
                    <span
                      className={`text-[10px] font-black uppercase tracking-[0.2em] px-4 py-1.5 rounded-lg ${member.riskStatus === "high" ? "bg-red-500/10 text-red-500 border border-red-500/20" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"}`}
                    >
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center bg-white p-4 rounded-3xl mx-auto my-2 mt-0">
                <QRCodeSVG
                  ...
                  size={140}
                />
                <p className="text-black font-bold uppercase tracking-[0.2em] text-[8px] mt-3">
                  Scan to check-in
                </p>
              </div>
...
function Scorecard({ title, value, trend, subtitle, highlight, icon }: any) {
  return (
    <div
      className={`p-8 rounded-[2.5rem] border relative overflow-hidden transition-all duration-700 group cursor-pointer font-bold ${highlight ? "bg-gold/10 border-gold/20 shadow-[0_0_50px_rgba(202,138,4,0.1)]" : "bg-[#121212]/30 border-white/5 hover:border-gold/20"}`}
    >
      ...
      <h3 className="text-[9px] font-black tracking-[0.3em] text-white/20 uppercase mb-5 group-hover:text-gold/50 transition-colors uppercase font-bold">
        {title}
      </h3>
      <div className="flex items-baseline gap-4">
        <span
          className={`text-5xl font-heading tracking-tight ${highlight ? "text-gold" : "text-white"}`}
        >
          {value}
        </span>
        {trend && (
          <span className="text-[10px] font-black text-emerald-400 bg-emerald-400/5 px-2 py-0.5 rounded italic">
            {trend}
          </span>
        )}
      </div>
