...
export default function KitchenTab() {
    ...
    const shakes = useMemo(() => kitchenItems.filter(item => item.category === 'shake'), [kitchenItems]);
    const meals = useMemo(() => kitchenItems.filter(item => item.category === 'meal'), [kitchenItems]);

    const handlePlaceOrder = async (item: any) => {
        try {
            await placeOrder([{ id: item.id, quantity: 1, name: item.name }], item.price);
        ...
        <div className="p-6 pt-12 flex flex-col gap-8 pb-32">
            ...
            <div className="flex flex-col gap-6">
                <div className="flex justify-between items-end px-1">
                    <h3 className="font-medium text-lg tracking-tight">Post-Recovery Shakes</h3>
                    <button className="text-[10px] text-gray-400 tracking-widest uppercase hover:text-[#FFB800] transition-colors">View All</button>
                </div>

                <div className="grid grid-cols-2 gap-4">
                    {shakes.map(item => (
                        <KitchenCard key={item.id} item={item} onOrder={() => handlePlaceOrder(item)} />
                    ))}
                </div>
            </div>
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-4">
                    {meals.map(item => (
                        <KitchenListItem key={item.id} item={item} onOrder={() => handlePlaceOrder(item)} />
                    ))}
                </div>
            </div>
            ...
            {orders.length > 0 ? (
                <div className="flex flex-col gap-4 bg-white/5 border border-white/10 p-6 rounded-3xl mt-4">
                    <h3 className="text-[10px] font-bold text-[#FFB800] tracking-widest uppercase mb-2">Recent Orders ({orders.length})</h3>
                    {orders.slice(0, 5).map(order => (
                        <div key={order.id} className="flex justify-between items-center bg-black/40 p-3 rounded-xl border border-white/5">
                            <span className="text-sm">Order #{order.id.slice(0, 4)}</span>
                            <span className="text-[9px] font-bold tracking-widest uppercase text-[#FFB800]">{order.status}</span>
                        </div>
                    ))}
                </div>
            ) : null}
    ...
    return (
        <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 overflow-hidden group hover:bg-white/10 transition-colors">
            <div className="h-32 relative overflow-hidden">
                <img src={item.image_url || "https://images.unsplash.com/photo-1610832958506-aa56368176cf?q=80&w=400&auto=format&fit=crop"} alt={item.name} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
...
function KitchenListItem({ item, onOrder }: any) {
    return (
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-3 flex gap-4 items-center group hover:bg-white/10 transition-colors">
            <img src={item.image_url || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=400&auto=format&fit=crop"} alt={item.name} className="w-20 h-20 rounded-xl object-cover" />
