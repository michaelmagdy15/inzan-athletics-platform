    ...
    return (
        <div className="p-6 pt-12 flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-2">
                <MoreItem icon={<Users size={20} />} label="Trainers" onClick={() => navigate('/p/trainers')} />
                <MoreItem icon={<Calendar size={20} />} label="Events & Offers" onClick={() => navigate('/p/events-offers')} />
                <MoreItem icon={<FileText size={20} />} label="Blogs" onClick={() => navigate('/p/blogs')} />
                <MoreItem icon={<MoreHorizontal size={20} />} label="Gallery" onClick={() => navigate('/p/gallery')} />
                <div className="h-px bg-white/10 my-2 mx-4" />
                <MoreItem icon={<Shield size={20} />} label="About INZAN" onClick={() => navigate('/p/about')} />
                <MoreItem icon={<Users size={20} />} label="Rules & Regulations" onClick={() => navigate('/p/rules')} />
                <div className="h-px bg-white/10 my-2 mx-4" />
                <MoreItem icon={<Users size={20} />} label="Contact Us" onClick={() => navigate('/p/contact')} />
...
function MoreItem({ icon, label, onClick }: { icon: React.ReactNode, label: string, onClick?: () => void }) {
    return (
        <div onClick={onClick} className="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/5 flex items-center justify-between cursor-pointer hover:bg-white/10 transition-all group">
