...
export default function MySessionsTab() {
  ...
  const [tab, setTab] = useState<"upcoming" | "packages" | "history">(
    "upcoming",
  );
  ...
  const history = mySessions.filter(
      ...
      <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10 backdrop-blur-md">
        {(["upcoming", "packages", "history"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex-1 py-2.5 text-[11px] uppercase tracking-widest font-bold rounded-xl transition-all ${tab === t ? "bg-white text-black shadow-lg" : "text-gray-400 hover:text-white"}`}
          >
            {t}
          </button>
        ))}
      ...
      {tab === "upcoming" && (
        <div className="flex flex-col gap-4">
          {upcoming.length === 0 && (
            <div className="text-center py-12 text-white/20">
              ...
              <p className="text-[10px] text-white/10 mt-1">
                Book a session from the PT tab
              </p>
            </div>
          )}
          {upcoming.map((session) => (
            <div
              key={session.id}
              className="bg-white/[0.03] border border-white/10 rounded-2xl p-5 flex flex-col gap-4"
            >
              <div className="flex justify-between items-start">
                ...
                <div
                  className={`px-3 py-1.5 rounded-full text-[9px] font-bold tracking-widest uppercase border ${STATUS_COLORS.scheduled}`}
                >
                  Scheduled
                </div>
              </div>
              <div className="flex gap-4 text-sm text-white/60">
                <span className="flex items-center gap-1">
                  <CalendarCheck size={14} className="text-[#FFB800]" />{" "}
      ...
      {tab === "history" && (
        <div className="flex flex-col gap-3">
          ...
          {history.map((session) => (
            <div
              key={session.id}
              className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 flex justify-between items-center"
            >
              <div>
              ...
              <div
                className={`px-3 py-1.5 rounded-full text-[8px] font-bold tracking-widest uppercase border ${STATUS_COLORS[session.status]}`}
              >
