    ...
    <div className="p-6 pt-12 flex flex-col gap-8">
      <header className="flex justify-between items-center">
        ...
        <button
          ...
          className="w-10 h-10 rounded-full bg-white/5 backdrop-blur-md border border-white/10 flex items-center justify-center hover:bg-white/10 relative transition-colors"
        >
          <Bell size={18} />
          {currentUser && (
            <span className="absolute top-2 right-2.5 w-2 h-2 bg-[#FFB800] rounded-full border-2 border-[#050505]" />
          )}
        </button>
      </header>
      ...
      <div className="bg-gradient-to-br from-[#1a1a1a] to-[#0a0a0a] rounded-3xl p-6 border border-white/10 relative overflow-hidden shadow-2xl">
        ...
        <p className="text-sm text-gray-300 leading-relaxed relative z-10 mb-5">
          {getRecoveryAdvice()} Your recovery is at{" "}
          <span
            className={`${currentUser?.recovery && currentUser.recovery < 50 ? "text-red-400" : "text-emerald-400"} font-bold`}
          >
        ...
        <div
          onClick={() => navigate("/p/gymnastics-kids")}
          className="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/5 flex justify-between items-center relative z-10 cursor-pointer hover:bg-white/10 transition-colors"
        >
          <div>
      ...
      {/* Hero Banner */}
      <div
        onClick={() => navigate("/p/request-freeze")}
        className="relative h-56 rounded-3xl overflow-hidden group cursor-pointer shadow-2xl"
      >
        <img
      ...
      <div
        onClick={() => navigate("/p/daily-schedule")}
        className="bg-white/5 backdrop-blur-xl rounded-3xl p-6 flex items-center justify-between border border-white/10 relative overflow-hidden cursor-pointer hover:bg-white/10 transition-all duration-300"
      >
