...
export default function NotificationBell() {
    ...
    const typeIcons: Record<string, string> = {
        ...
        reminder: '⏰', capacity: '📊', no_show: '⚠️', info: 'ℹ️',
    };
                ...
                <>
                    ...
                    <div className="absolute right-0 top-12 w-80 max-h-96 overflow-y-auto bg-[#111] border border-white/10 rounded-2xl shadow-2xl z-50">
                        <div className="flex items-center justify-between p-4 border-b border-white/5">
                            <h4 className="text-[10px] font-black text-white/40 tracking-[0.3em] uppercase">Notifications</h4>
                            {unreadCount > 0 && (
                                <button onClick={() => { markAllNotificationsRead(); }} className="text-[9px] text-[#FFB800] font-bold tracking-widest uppercase flex items-center gap-1 hover:text-white transition-colors">
                                    <CheckCheck size={12} />
                                    Mark all
                                </button>
                            )}
                        ...
                        {notifications.length === 0 && (
                            <p className="text-center text-white/20 text-sm py-8">No notifications</p>
                        )}
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                        <p className={`text-xs font-bold truncate ${n.is_read ? 'text-white/40' : 'text-white'}`}>{n.title}</p>
                                        {!n.is_read && <div className="w-2 h-2 rounded-full bg-[#FFB800] shrink-0" />}
                                    </div>
                                    <p className="text-[10px] text-white/30 mt-1 line-clamp-2">{n.message}</p>
